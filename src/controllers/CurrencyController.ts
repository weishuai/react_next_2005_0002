...
//import { ParameterVo } from '../stores/Parameter'; currency
export class CurrencyController extends BaseController {
  ...
  async remove(Ids: string[]) {
    ...
    const listItems = numbers.map((id) => {
      return fetch(this.baseURL()+'/currency/removeCurrency/'+id).then(res => res.json())
      .then(d => d);
    });
    ///this.success();
  }
  ...
  async getById(Id:string) {
    ...
    return fetch(this.baseURL()+'/currency/getCurrencyById/'+Id).then(res => res.json())
            .then(d => d);
  ...
  async getAll(fhevent:any) {
    ...
    return fetch(this.baseURL()+'/currency/getCurrencyAll?'+fhurl).then(res => res.json())
            .then(d => d);
  ...
  async getAllView() {
    ...
    return fetch(this.baseURL()+'/currency/getCurrencyAllView').then(res => res.json())
            .then(d => d);
  }
  ...
  async update(data:any,Id:string,userToken:string) {
    ...
    await axios({
         baseURL:this.baseURL(),
         method:'put',
         url:'/currency/updateCurrency/'+Id,
