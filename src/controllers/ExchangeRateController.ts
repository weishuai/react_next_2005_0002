...
//import { ParameterVo } from '../stores/Parameter'; exchangeRate
export class ExchangeRateController extends BaseController {
  ...
  async remove(Ids: string[]) {
    ...
    const listItems = numbers.map((id) => {
      return fetch(this.baseURL()+'/exchangeRate/removeExchangeRate/'+id).then(res => res.json())
      .then(d => d);
    });
    ///this.success();
  }
  ...
  async getById(Id:string) {
    ...
    return fetch(this.baseURL()+'/exchangeRate/getExchangeRateById/'+Id).then(res => res.json())
            .then(d => d);
  ...
  async getAll(fhevent:any) {
    ...
    return fetch(this.baseURL()+'/exchangeRate/getExchangeRateAll?'+fhurl).then(res => res.json())
            .then(d => d);
  ...
  async getAllView() {
    ...
    return fetch(this.baseURL()+'/exchangeRate/getExchangeRateAllView').then(res => res.json())
            .then(d => d);
  }
  ...
  async update(data:any,Id:string,userToken:string) {
    ...
    await axios({
         baseURL:this.baseURL(),
         method:'put',
         url:'/exchangeRate/updateExchangeRate/'+Id,
