...
export const QuotationList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlQuotation.getQuotationAll(fhevent).then(data =>{
            ...
            //  ctlQuotation.getQuotationAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/quotation/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlQuotation.removeQuotation([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/quotation");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("quotation.name")}  field = "name" sortable key =  {10001}   ></Column>,
                            <Column header ={t("quotation.ref_number")}  field = "ref_number" sortable key =  {10005}   ></Column>,
                            <Column header ={t("quotation.opportunity_id")}  field = "opportunity_id" sortable key =  {10006}   ></Column>,
                            <Column header ={t("quotation.quotation_name")}  field = "quotation_name" sortable key =  {10007}   ></Column>,
                            <Column header ={t("quotation.contact")}  field = "contact" sortable key =  {100010}   ></Column>,

                            <Column header ={t("quotation.status")}  field = "status" sortable key =  {100014}   ></Column>,
