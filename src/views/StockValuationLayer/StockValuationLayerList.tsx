...
export const StockValuationLayerList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlStockValuationLayer.getStockValuationLayerAll(fhevent).then(data =>{

             ctlStockValuationLayer.getStockValuationLayerAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/stockValuationLayer/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlStockValuationLayer.removeStockValuationLayer([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/stockValuationLayer");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("stockValuationLayer.fhdate")} field = "fhdate" sortable key =  {10005}  ></Column>,
                            <Column header ={t("stockValuationLayer.product")} field = "product" sortable key =  {10006} ></Column>,
                            <Column header ={t("stockValuationLayer.number")} field = "number" sortable key =  {10007}  ></Column>,
                            <Column header ={t("stockValuationLayer.unit")} field = "unit" sortable key =  {10008} ></Column>,
                            <Column header ={t("stockValuationLayer.totalValue")} field = "totalValue" sortable key =  {10009}  ></Column>,
                            <Column header ={t("stockValuationLayer.company")} field = "company" sortable key =  {100010} ></Column>,
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/stockValuationLayer/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/stockValuationLayer/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlStockValuationLayer.removeStockValuationLayer([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
