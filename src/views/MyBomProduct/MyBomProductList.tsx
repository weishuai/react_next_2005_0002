...
export const MyBomProductList = () => {
    ...
    const ctlMyBomProduct = new MyBomProductController();
    ...
    const fn=async (fhevent)=>{
        await   ctlMyBomProduct.getMyBomProductAll(fhevent).then(data =>{

            //  ctlMyBomProduct.getMyBomProductAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/mybomProduct/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlMyBomProduct.removeMyBomProduct([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/mybomProduct");
                }}   />
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("mybomProduct.product")}  field = "product" sortable key =  {10006}  ></Column>,
                            <Column header ={t("mybomProduct.number")} field = "number" sortable key =  {10007}  ></Column>,
                            <Column header ={t("mybomProduct.unit")}  field = "unit" sortable key =  {10008}  ></Column>,

                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/mybomProduct/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/mybomProduct/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlMyBomProduct.removeMyBomProduct([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
