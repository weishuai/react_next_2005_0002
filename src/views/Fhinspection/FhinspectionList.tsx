...
export const FhinspectionList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlFhinspection.getFhinspectionAll(fhevent).then(data =>{

             ctlFhinspection.getFhinspectionAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/fhinspection/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlFhinspection.removeFhinspection([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/fhinspection");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("fhinspection.fhname")}  field = "fhname" sortable key =  {10005}  ></Column>,
                            <Column header ={t("fhinspection.fhF1")} field = "fhF1" sortable key =  {10006}  ></Column>,
                            <Column header ={t("fhinspection.fhF2")} field = "fhF2" sortable key =  {10007}  ></Column>,
                            <Column header ={t("fhinspection.fhF3")}  field = "fhF3" sortable key =  {10008}  ></Column>,
                            <Column header ={t("fhinspection.fhF4")} field = "fhF4" sortable key =  {10009}  ></Column>,
                         

                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/fhinspection/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/fhinspection/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlFhinspection.removeFhinspection([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
