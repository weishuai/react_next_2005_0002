...
export const SellingPriceListList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlSellingPriceList.getSellingPriceListAll(fhevent).then(data =>{

             ctlSellingPriceList.getSellingPriceListAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/pages/sellingPriceList/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlSellingPriceList.removeSellingPriceList([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/sellingPriceList");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("sellingPriceList.mid")}  field = "mid" sortable key =  {10005}  ></Column>,
                            <Column header ={t("sellingPriceList.product")}  field = "product" sortable key =  {10006}  ></Column>,
                            <Column header ={t("sellingPriceList.variant")}  field = "variant" sortable key =  {10007}  ></Column>,
                            <Column header ={t("sellingPriceList.miniMumQuantity")} field = "miniMumQuantity" sortable key =  {10008}  ></Column>,
                            <Column header ={t("sellingPriceList.price")} field = "price" sortable key =  {10009}  ></Column>,
                            <Column header ={t("sellingPriceList.startDate")} field = "startDate" sortable key =  {100010}  ></Column>,
                            <Column header ={t("sellingPriceList.endDate")} field = "endDate" sortable key =  {100011}  ></Column>,
                          ...
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/sellingPriceList/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/sellingPriceList/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlSellingPriceList.removeSellingPriceList([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
