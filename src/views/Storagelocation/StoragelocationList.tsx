...
export const StoragelocationList = () => {
    ...
    const ctlStoragelocation = new StoragelocationController();
    const ctlTiems=new TimesController();
    const { pathname, push,replace } = useRouter();
    const [val, setVal] = useState("");
    var fhcount=0;
    const fn=async (fhevent)=>{
        await   ctlStoragelocation.getStoragelocationAll(fhevent).then(data =>{

             ctlStoragelocation.getStoragelocationAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/storagelocation/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlStoragelocation.removeStoragelocation([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/storagelocation");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("storagelocation.myname")}  field = "myname" sortable key =  {10005}  ></Column>,
                            <Column header ={t("storagelocation.mid")}  field = "mid" sortable key =  {10006}  ></Column>,
                            <Column header ={t("storagelocation.type")}  field = "type" sortable key =  {10007}  ></Column>,

    
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/storagelocation/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/storagelocation/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlStoragelocation.removeStoragelocation([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
