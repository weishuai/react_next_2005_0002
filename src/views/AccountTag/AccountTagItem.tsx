...
export type AccountTagItemProps = {
  Id: string;
  ///0 新增 1 编辑 2 查看
  Mode: string;
};
export function AccountTagItem({ Id, Mode }: AccountTagItemProps) {
    console.log('Mode:'+ Mode);
    if (Mode == '0') {
      return <AccountTagItemAdd  Id={Id} Mode={Mode}/>;
    } 
    else if(Mode == '1')
    {
      return <AccountTagItemEdit Id={Id} Mode={Mode} />;
    } 
    else if(Mode == '2')
    {
      return <AccountTagItemView Id={Id} Mode={Mode} />;
    }
    else
    {
      return <AccountTagItemAdd  Id={Id} Mode={Mode}/>;
...
export const AccountTagItemAdd =({ Id, Mode }: AccountTagItemProps) => {
  ...
  <Card>     
      ...
      <tbody>
          ...
          <td >
                <div className="p-field p-col-12 p-md-6">
                <div style={{height:10}}> </div>
                <Button label="取消" onClick={(e) => {replace("/account");}}  style={{backgroundColor:'#4682B4'}}  />
                </div>
          </td>
          <td ></td>
          <td>
                ...
                <div style={{height:10}}> </div>
                  <Button
                    label="提交"
                    onClick={(e) => { 
                      ctlAccountTag.createAccountTag({
                        id:FHid,
                        name:name,
...
export const AccountTagItemEdit =({ Id, Mode }: AccountTagItemProps) => {
  const [activeIndex, setActiveIndex] = useState(0);
  const [islocked, setIslocked] = useState("0");
  const [isactived, setIsactived] = useState("0");
  const [createUid, setCreateUid] = useState(null);
  ...
  const [FHid, setFHid] = useState('');
    ...
    useEffect(() => {
      ...
      */
      ...
      getFHusers().then(async (data)=>{
        console.log('data:'+JSON.stringify(data));
        setFHUsers(data);
        await   ctlAccountTag.getAccountTagById(Id).then((data)=>{
                ...
                <div style={{height:10}}> </div>
                  <Button
                    ...
                    onClick={(e) => { 
                      ...
                      const userID=globalStorage.get("userID");
                      console.log('userID:'+userID);
                      ctlAccountTag.updateAccountTagv2({
                        id:FHid,
                        name:name,
                        description:description,
                        isactived:isactived,
...
export const AccountTagItemView =({ Id, Mode }: AccountTagItemProps) => {
  const [activeIndex, setActiveIndex] = useState(0);
  const [islocked, setIslocked] = useState("0");
  const [isactived, setIsactived] = useState("0");
  const [createUid, setCreateUid] = useState(null);
