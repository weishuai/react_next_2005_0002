...
export const MybomList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlMybom.getMybomAll(fhevent).then(data =>{

             ctlMybom.getMybomAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/mybom/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlMybom.removeMybom([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/mybom");
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("mybom.product")}  field = "product" sortable key =  {10005}  ></Column>,
                            <Column header ={t("mybom.code")}  field = "code" sortable key =  {10006}  ></Column>,
                            <Column header ={t("mybom.productTelm")}  field = "productTelm" sortable key =  {10007}  ></Column>,
                            <Column header ={t("mybom.bomType")}  field = "bomType" sortable key =  {10008}  ></Column>,
                            <Column header ={t("mybom.number")} field = "number" sortable key =  {10009}  ></Column>,

                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/mybom/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/mybom/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlMybom.removeMybom([rowData.id]).then(()=>{
                                             // fn(fhevent);
                                            });
