...
export const ReturnorderList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlReturnorder.getReturnorderAll(fhevent).then(data =>{

             ctlReturnorder.getReturnorderAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/pages/returnorder/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlReturnorder.removeReturnorder([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/returnorder");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("returnorder.project")}  field = "project" sortable key =  {10005}  ></Column>,
                            <Column header ={t("returnorder.custom")}  field = "custom" sortable key =  {10006}  ></Column>,
                            <Column header ={t("returnorder.Invoice")}  field = "Invoice" sortable key =  {10007}  ></Column>,
                            <Column header ={t("returnorder.contractNumber")}  field = "contractNumber" sortable key =  {10008}  ></Column>,
                            <Column header ={t("returnorder.contractQuantity")} field = "contractQuantity" sortable key =  {10009}  ></Column>,
                            <Column header ={t("returnorder.product")}  field = "product" sortable key =  {100010}  ></Column>,
                            <Column header ={t("returnorder.shipper")}  field = "shipper" sortable key =  {100011}  ></Column>,
                            <Column header ={t("returnorder.deliveryQuantity")} field = "deliveryQuantity" sortable key =  {100012}  ></Column>,
                            <Column header ={t("returnorder.outageQuantity")} field = "outageQuantity" sortable key =  {100013}  ></Column>,
                            <Column header ={t("returnorder.deliveryType")}  field = "deliveryType" sortable key =  {100014}  ></Column>,
                            <Column header ={t("returnorder.trackingNumber")}  field = "trackingNumber" sortable key =  {100015}  ></Column>,
                            <Column header ={t("returnorder.recipient")}  field = "recipient" sortable key =  {100016}  ></Column>,
                          ...
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/returnorder/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/returnorder/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlReturnorder.removeReturnorder([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
