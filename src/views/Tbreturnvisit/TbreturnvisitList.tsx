...
export const TbreturnvisitList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlTbreturnvisit.getTbreturnvisitAll(fhevent).then(data =>{

             ctlTbreturnvisit.getTbreturnvisitAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/pages/tbreturnvisit/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlTbreturnvisit.removeTbreturnvisit([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/tbreturnvisit");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("tbreturnvisit.mid")}  field = "mid" sortable key =  {10005}  ></Column>,
                            <Column header ={t("tbreturnvisit.project")}  field = "project" sortable key =  {10006}  ></Column>,
                            <Column header ={t("tbreturnvisit.fhdate")} field = "fhdate" sortable key =  {10007}  ></Column>,
                            <Column header ={t("tbreturnvisit.location")}  field = "location" sortable key =  {10008}  ></Column>,
                            <Column header ={t("tbreturnvisit.contacts")}  field = "contacts" sortable key =  {10009}  ></Column>,
                            <Column header ={t("tbreturnvisit.contactInformation")}  field = "contactInformation" sortable key =  {100010}  ></Column>,
                            <Column header ={t("tbreturnvisit.rreturnVisitDescription")}  field = "rreturnVisitDescription" sortable key =  {100011}  ></Column>,
                            <Column header ={t("tbreturnvisit.annex")}  field = "annex" sortable key =  {100012}  ></Column>,
                          ...
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/tbreturnvisit/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/tbreturnvisit/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlTbreturnvisit.removeTbreturnvisit([rowData.id]).then(()=>{
                                             // fn(fhevent);
                                            });
