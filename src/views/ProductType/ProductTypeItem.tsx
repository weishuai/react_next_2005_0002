...
export type ProductTypeItemProps = {
  Id: string;
  ///0 新增 1 编辑 2 查看
  Mode: string;
};
export function ProductTypeItem({ Id, Mode }: ProductTypeItemProps) {
    console.log('Mode:'+ Mode);
    if (Mode == '0') {
      return <ProductTypeItemAdd  Id={Id} Mode={Mode}/>;
    } 
    else if(Mode == '1')
    {
      return <ProductTypeItemEdit Id={Id} Mode={Mode} />;
    } 
    else if(Mode == '2')
    {
      return <ProductTypeItemView Id={Id} Mode={Mode} />;
    }
    else
    {
      return <ProductTypeItemAdd  Id={Id} Mode={Mode}/>;
...
export const ProductTypeItemAdd =({ Id, Mode }: ProductTypeItemProps) => {
  ...
  <Card>     
      ...
      <tbody>
          ...
          <td colSpan={4}>      
                ...
                <label htmlFor="name">{t("producttype.name")}</label>
          ...
          <tr>
            <td colSpan={4}>
                ...
                <label htmlFor="firstname1">{t("producttype.description")}</label>
                <div style={{height:10}}> </div>
                <InputTextarea 
                id="description"
                value={description}  
          ...
          <td  width="46%">
                ...
                <label htmlFor="firstname1">{t("producttype.isactived")}</label>
                <div style={{height:10}}> </div>
                <Dropdown 
                value={isactived} 
                disabled={true}
                ...
                }}  placeholder={t("producttype.isactived")} />
                </div>
          </td>
          <td width="2%"></td> 
          <td  width="46%">
                <div className="p-field p-col-12 p-md-6">
                <label htmlFor="firstname1">{t("producttype.islocked")}</label>
                <div style={{height:10}}> </div>
                <Dropdown 
                value={islocked} 
                disabled={true}
                ...
                onChange={(e: { value: any}) => {
                 console.info('e.value:'+JSON.stringify(e.value));
                 setIslocked(e.value);
                }}  placeholder={t("producttype.islocked")} />
                </div> 
          </td>
          </tr>
          <tr>
          ...
          <td >
                <div className="p-field p-col-12 p-md-6">
                <label htmlFor="firstname1">{t("producttype.createUid")}</label>
                <div style={{height:10}}> </div>
                <Dropdown 
                value={createUid} 
                disabled={true}
                ...
                }}  placeholder={t("producttype.createUid")} />
          ...
          <td >
                <div className="p-field p-col-12 p-md-6">
                <label htmlFor="firstname1">{t("producttype.updatedUid")}</label>
                <div style={{height:10}}> </div>
                <Dropdown 
                value={updatedUid} 
                disabled={true}
                ...
                }}  placeholder={t("producttype.updatedUid")} />
               </div>  
          </td>
          </tr>
          ...
          <td >
                <div className="p-field p-col-12 p-md-6">
                <div style={{height:10}}> </div>
                <Button label="取消" onClick={(e) => {replace("/productType");}}  style={{backgroundColor:'#4682B4'}}  />
                </div>
          </td>
          <td ></td>
          <td>
                ...
                <div style={{height:10}}> </div>
                  <Button
                    label="提交"
                    onClick={(e) => { 
                      ctlProductType.createProductType({
                        id:FHid,
                        name:name,
...
export const ProductTypeItemEdit =({ Id, Mode }: ProductTypeItemProps) => {
  const [activeIndex, setActiveIndex] = useState(0);
  const [activeIndex1, setActiveIndex1] = useState(0);
  const [islocked, setIslocked] = useState("0");
  const [isactived, setIsactived] = useState("0");
   ...
   const items = [
    ...
    useEffect(() => {
      ...
      getFHusers().then((data)=>{
        console.log('data:'+JSON.stringify(data));
        setFHUsers(data);
          ctlProductType.getProductTypeById(Id).then((data)=>{
                ...
                <div style={{height:10}}> </div>
                  <Button
                    ...
                    onClick={(e) => { 
                      ...
                      const userID=globalStorage.get("userID");
                      console.log('userID:'+userID);
                      ctlProductType.updateProductTypev2({
                        id:FHid,
                        name:name,
                        description:description,
                        isactived:isactived,
...
export const ProductTypeItemView =({ Id, Mode }: ProductTypeItemProps) => {
  const [activeIndex, setActiveIndex] = useState(0);
  const [activeIndex1, setActiveIndex1] = useState(0);
  const [islocked, setIslocked] = useState("0");
  const [isactived, setIsactived] = useState("0");
