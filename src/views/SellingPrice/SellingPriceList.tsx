...
export const SellingPriceList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlSellingPrice.getSellingPriceAll(fhevent).then(data =>{

             ctlSellingPrice.getSellingPriceAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/sellingprice/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlSellingPrice.removeSellingPrice([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/sellingPrice");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("sellingPrice.fhname")}  field = "fhname" sortable key =  {10005}  ></Column>,
                            <Column header ={t("sellingPrice.company")} field = "company" sortable key =  {10006}  ></Column>,
                            <Column header ={t("sellingPrice.mygroup")} field = "mygroup" sortable key =  {10007}  ></Column>,
                            <Column header ={t("sellingPrice.dscountPolicy")}  field = "dscountPolicy" sortable key =  {10008}  ></Column>,


                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/sellingPrice/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/sellingPrice/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlSellingPrice.removeSellingPrice([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
