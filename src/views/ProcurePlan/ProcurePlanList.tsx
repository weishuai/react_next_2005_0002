...
export const ProcurePlanList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlProcurePlan.getProcurePlanAll(fhevent).then(data =>{

             ctlProcurePlan.getProcurePlanAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/procureplan/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlProcurePlan.removeProcurePlan([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/procurePlan");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("procurePlan.fhname")}  field = "fhname" sortable key =  {10005}  ></Column>,
                            <Column header ={t("procurePlan.purchaser")}  field = "purchaser" sortable key =  {10006}  ></Column>,
                            <Column header ={t("procurePlan.applicationType")}  field = "applicationType" sortable key =  {10007}  ></Column>,
                            <Column header ={t("procurePlan.supplier")}  field = "supplier" sortable key =  {10008}  ></Column>,
                            <Column header ={t("procurePlan.deadline")} field = "deadline" sortable key =  {10009}  ></Column>,
              
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/procurePlan/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/procurePlan/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlProcurePlan.removeProcurePlan([rowData.id]).then(()=>{
                                             // fn(fhevent);
                                            });
