...
export const TbaftersaleList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlTbaftersale.getTbaftersaleAll(fhevent).then(data =>{

             ctlTbaftersale.getTbaftersaleAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/pages/tbaftersale/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlTbaftersale.removeTbaftersale([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/tbaftersale");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("tbaftersale.mid")}  field = "mid" sortable key =  {10005}  ></Column>,
                            <Column header ={t("tbaftersale.project")}  field = "project" sortable key =  {10006}  ></Column>,
                            <Column header ={t("tbaftersale.fhdate")}  field = "fhdate" sortable key =  {10007}  ></Column>,
                            <Column header ={t("tbaftersale.location")}  field = "location" sortable key =  {10008}  ></Column>,
                            <Column header ={t("tbaftersale.contacts")}  field = "contacts" sortable key =  {10009}  ></Column>,
                            <Column header ={t("tbaftersale.contactInformation")}  field = "contactInformation" sortable key =  {100010}  ></Column>,
                            <Column header ={t("tbaftersale.VisitDescription")}  field = "VisitDescription" sortable key =  {100011}  ></Column>,
                            <Column header ={t("tbaftersale.problemHandling")}  field = "problemHandling" sortable key =  {100012}  ></Column>,
                            <Column header ={t("tbaftersale.remainingProblems")}  field = "remainingProblems" sortable key =  {100013}  ></Column>,
                            <Column header ={t("tbaftersale.afterSalesType")}  field = "afterSalesType" sortable key =  {100014}  ></Column>,
                            <Column header ={t("tbaftersale.annex")}  field = "annex" sortable key =  {100015}  ></Column>,
                            <Column header ={t("tbaftersale.processedBy")}  field = "processedBy" sortable key =  {100016}  ></Column>,
                          ...
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/tbaftersale/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/tbaftersale/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlTbaftersale.removeTbaftersale([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
