...
export const MyWorkCenterList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlMyWorkCenter.getMyWorkCenterAll(fhevent).then(data =>{

             ctlMyWorkCenter.getMyWorkCenterAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/myworkcenter/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlMyWorkCenter.removeMyWorkCenter([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/myworkCenter");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("myworkCenter.workcenter")}  field = "workcenter" sortable key =  {10005}  ></Column>,
                            <Column header ={t("myworkCenter.code")}  field = "code" sortable key =  {10006}  ></Column>,
                            <Column header ={t("myworkCenter.title")}  field = "title" sortable key =  {10007}  ></Column>,
                            <Column header ={t("myworkCenter.workingHours")} field = "workingHours" sortable key =  {10008}  ></Column>,
                            <Column header ={t("myworkCenter.fhreplace")}  field = "fhreplace" sortable key =  {10009}  ></Column>,
                            <Column header ={t("myworkCenter.timeEfficiency")}  field = "timeEfficiency" sortable key =  {100010}  ></Column>,
                            <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/myworkCenter/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/myworkCenter/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlMyWorkCenter.removeMyWorkCenter([rowData.id]).then(()=>{
                                              fn(fhevent);
                                            });
                                         }}  />
                                    </React.Fragment>
