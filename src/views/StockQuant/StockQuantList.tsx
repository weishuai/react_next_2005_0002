...
export const StockQuantList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlStockQuant.getStockQuantAll(fhevent).then(data =>{

             ctlStockQuant.getStockQuantAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/stockQuant/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlStockQuant.removeStockQuant([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/stockQuant");
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("stockQuant.product")}  field = "product" sortable key =  {10005}  ></Column>,
                            <Column header ={t("stockQuant.position")}  field = "position" sortable key =  {10006}  ></Column>,
                            <Column header ={t("stockQuant.batch")}  field = "batch" sortable key =  {10007}  ></Column>,
                            <Column header ={t("stockQuant.myPackage")}  field = "package" sortable key =  {10008}  ></Column>,
                            <Column header ={t("stockQuant.inHand")} field = "inHand" sortable key =  {10009}  ></Column>,
                            <Column header ={t("stockQuant.available")} field = "available" sortable key =  {100010}  ></Column>,
                            <Column header ={t("stockQuant.unit")}  field = "unit" sortable key =  {100011}  ></Column>,
                            <Column header ={t("stockQuant.price")} field = "price" sortable key =  {100012}  ></Column>,
                            <Column header ={t("stockQuant.company")}  field = "company" sortable key =  {100013}  ></Column>,

                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/stockQuant/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/stockQuant/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlStockQuant.removeStockQuant([rowData.id]).then(()=>{
                                             // fn(fhevent);
                                            });
