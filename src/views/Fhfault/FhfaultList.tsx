...
export const FhfaultList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlFhfault.getFhfaultAll(fhevent).then(data =>{

             ctlFhfault.getFhfaultAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/fhfault/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlFhfault.removeFhfault([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/fhfault");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("fhfault.fhname")}  field = "fhname" sortable key =  {10005}  ></Column>,
                            <Column header ={t("fhfault.fhF1")} field = "fhF1" sortable key =  {10006}  ></Column>,
                            <Column header ={t("fhfault.fhF2")} field = "fhF2" sortable key =  {10007}  ></Column>,
                            <Column header ={t("fhfault.fhF3")}  field = "fhF3" sortable key =  {10008}  ></Column>,
                           
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/fhfault/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/fhfault/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlFhfault.removeFhfault([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
