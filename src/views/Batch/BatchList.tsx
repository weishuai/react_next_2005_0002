...
export const BatchList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlBatch.getBatchAll(fhevent).then(data =>{

             ctlBatch.getBatchAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/batch/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlBatch.removeBatch([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/batch");
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("batch.titel")}  field = "titel" sortable key =  {10005}  ></Column>,
                            <Column header ={t("batch.product")}  field = "product" sortable key =  {10006}  ></Column>,
                            <Column header ={t("batch.number")}  field = "number" sortable key =  {10007}  ></Column>,
                            <Column header ={t("batch.code")}  field = "code" sortable key =  {10008}  ></Column>,
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/batch/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/batch/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlBatch.removeBatch([rowData.id]).then(()=>{
                                             // fn(fhevent);
                                            });
