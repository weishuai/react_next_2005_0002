...
export const FhmeasuringList = () => {
    ...
    const ctlFhmeasuring = new FhmeasuringController();
    const ctlTiems=new TimesController();
    const { pathname, push,replace } = useRouter();
    const [val, setVal] = useState("");
    var fhcount=0;
    const fn=async (fhevent)=>{
        await   ctlFhmeasuring.getFhmeasuringAll(fhevent).then(data =>{

             ctlFhmeasuring.getFhmeasuringAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/fhmeasuring/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlFhmeasuring.removeFhmeasuring([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/fhmeasuring");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("fhmeasuring.fhname")}  field = "fhname" sortable key =  {10005}  ></Column>,
                            <Column header ={t("fhmeasuring.fhF1")} field = "fhF1" sortable key =  {10006}  ></Column>,
                            <Column header ={t("fhmeasuring.fhF2")} field = "fhF2" sortable key =  {10007}  ></Column>,
                            <Column header ={t("fhmeasuring.fhF3")}  field = "fhF3" sortable key =  {10008}  ></Column>,
                            <Column header ={t("fhmeasuring.fhF4")} field = "fhF4" sortable key =  {10009}  ></Column>,
                            <Column header ={t("fhmeasuring.fhF5")} field = "fhF5" sortable key =  {100010}  ></Column>,
                           

                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/fhmeasuring/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/fhmeasuring/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlFhmeasuring.removeFhmeasuring([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
