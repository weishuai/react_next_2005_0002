...
export const WarehouseList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlWarehouse.getWarehouseAll(fhevent).then(data =>{

             ctlWarehouse.getWarehouseAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/warehouse/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlWarehouse.removeWarehouse([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/warehouse");
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("warehouse.myname")}  field = "myname" sortable key =  {10005}  ></Column>,
                            <Column header ={t("warehouse.fhname")}  field = "fhname" sortable key =  {10006}  ></Column>,
                            <Column header ={t("warehouse.address")}  field = "address" sortable key =  {10007}  ></Column>,
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/warehouse/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/warehouse/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlWarehouse.removeWarehouse([rowData.id]).then(()=>{
                                             // fn(fhevent);
                                            });
