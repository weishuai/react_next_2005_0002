...
export const StoreMoveListList = () => {
    ...
    const ctlStoreMoveList = new StoreMoveListController();
    const ctlTiems=new TimesController();
    const { pathname, push,replace } = useRouter();
    const [val, setVal] = useState("");
    var fhcount=0;
    const fn=async (fhevent)=>{
        await   ctlStoreMoveList.getStoreMoveListAll(fhevent).then(data =>{

             ctlStoreMoveList.getStoreMoveListAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/stormovelist/0/0");}} style={{backgroundColor:'#4682B4'}}  />
          
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlStoreMoveList.removeStoreMoveList([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/storeMoveList");
                }}   />
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("storeMoveList.product")}  field = "product" sortable key =  {10006}  ></Column>,
                            <Column header ={t("storeMoveList.demand")} field = "demand" sortable key =  {10007}  ></Column>,
                            <Column header ={t("storeMoveList.reserve")} field = "reserve" sortable key =  {10008}  ></Column>,
                            <Column header ={t("storeMoveList.complete")} field = "complete" sortable key =  {10009}  ></Column>,
                            <Column header ={t("storeMoveList.unit")}  field = "unit" sortable key =  {100010}  ></Column>,
                            <Column header ={t("storeMoveList.sellingPrice")} field = "sellingPrice" sortable key =  {100011}  ></Column>,
                            <Column header ={t("storeMoveList.costPprice")} field = "costPprice" sortable key =  {100012}  ></Column>,
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/storeMoveList/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/storeMoveList/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlStoreMoveList.removeStoreMoveList([rowData.id]).then(()=>{
                                             // fn(fhevent);
                                            });
