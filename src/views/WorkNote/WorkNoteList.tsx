...
import {WorkNoteController} from '../../controllers/WorkNoteController';
import {Button} from 'primereact/button';
import {MyDataTable} from '../../components/myDataTable/DataTable';
const { nanoid } = require('nanoid');
export const WorkNoteList = (props: any) => {
    const { t, i18n } = useTranslation();
    const [customers, setCustomers] = React.useState(null);
    const [FHids, setFHids] = useState([]);
    const ctlWorkNote = new WorkNoteController();
    ...
    const fn=async ()=>{
        ...
        await   ctlWorkNote.getWorkNoteAll(fhevent).then(data =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/worknote/0/0/"+Mainid);}} style={{backgroundColor:'#4682B4'}}  />
         
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlWorkNote.removeWorkNote([item.id]).then(()=>{});
                }
                //replace("/worknote");
                 ...
                 fhHeight={'800px'}                    
                    FHColumns={
                        [
                            ...
                            // <Column header={t('workNote.name')} field="name" sortable  key={2}></Column>,
                            <Column header ={t("workNote.title")}  field = "title" sortable key =  {10005}  ></Column>,
                            <Column header ={t("workNote.assignTo")}  field = "assignTo" sortable key =  {10006}  ></Column>,
                            ...
                            <Column  header={t('dataTable.Info')} field="id"   alignHeader="center" headerStyle={{minWidth: '14rem'}} style={{ width: '14rem'}}   body={(rowData) => {
                                return(
                                    <div style={{textAlign:'center',flex:1}}>
                                      <Button label={t('dataTable.HeadInfo')} style={{height:25,width:51}} className="p-button-sm p-button-info"  onClick={()=>{replace("/worknote/"+rowData.id+"/2/"+props.mid);}}></Button>
                                                                           
                                      <Button label={t('dataTable.Edit')}  style={{height:25,width:51}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/worknote/"+rowData.id+"/1/"+props.mid);}} />

                                      <Button label={t('dataTable.Delete')}  style={{height:25,width:51}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlWorkNote.removeWorkNote([rowData.id]).then(()=>{
                                             //replace("/worknote");
                                            });
                                            const mynanoid: string = nanoid();
                                            props.FHonChange(mynanoid);
