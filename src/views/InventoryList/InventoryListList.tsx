...
export const InventoryListList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlInventoryList.getInventoryListAll(fhevent).then(data =>{

             ctlInventoryList.getInventoryListAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/inventoryList/0/0");}} style={{backgroundColor:'#4682B4'}}  />

            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlInventoryList.removeInventoryList([item.id]).then(()=>{
                     //fn(fhevent);
                    });
                }
                //replace("/inventoryList");
                }}   />
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("inventoryList.product")}  field = "product" sortable key =  {10006}  ></Column>,
                            <Column header ={t("inventoryList.unit")}  field = "unit" sortable key =  {10007}  ></Column>,
                            <Column header ={t("inventoryList.position")}  field = "position" sortable key =  {10008}  ></Column>,
                            <Column header ={t("inventoryList.batch")}  field = "batch" sortable key =  {10009}  ></Column>,
                            <Column header ={t("inventoryList.package")}  field = "package" sortable key =  {100010}  ></Column>,
                            <Column header ={t("inventoryList.owner")}  field = "owner" sortable key =  {100011}  ></Column>,
                            <Column header ={t("inventoryList.theoretical")} field = "theoretical" sortable key =  {100012}  ></Column>,
                            <Column header ={t("inventoryList.actualQuantity")} field = "actualQuantity" sortable key =  {100013}  ></Column>,


                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/inventoryList/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/inventoryList/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlInventoryList.removeInventoryList([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
