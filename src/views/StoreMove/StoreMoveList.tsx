...
export const StoreMoveList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlStoreMove.getStoreMoveAll(fhevent).then(data =>{

             ctlStoreMove.getStoreMoveAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/storemove/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlStoreMove.removeStoreMove([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/storeMove");
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("storeMove.partners")}  field = "partners" sortable key =  {10005}  ></Column>,
                            <Column header ={t("storeMove.deliveryDate")} field = "deliveryDate" sortable key =  {10006}  ></Column>,
                            <Column header ={t("storeMove.sourceLocation")}  field = "sourceLocation" sortable key =  {10007}  ></Column>,
                            <Column header ={t("storeMove.sourceDocument")}  field = "sourceDocument" sortable key =  {10008}  ></Column>,
                            <Column header ={t("storeMove.type")}  field = "type" sortable key =  {10009}  ></Column>,
                            <Column header ={t("storeMove.owner")}  field = "owner" sortable key =  {100010}  ></Column>,
                            <Column header ={t("storeMove.fhtime")} field = "fhtime" sortable key =  {100011}  ></Column>,
                            <Column header ={t("storeMove.notes")}  field = "notes" sortable key =  {100012}   ></Column>,
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/storeMove/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/storeMove/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlStoreMove.removeStoreMove([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
