...
export const TbpackagList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlTbpackag.getTbpackagAll(fhevent).then(data =>{

             ctlTbpackag.getTbpackagAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/pages/tbpackag/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlTbpackag.removeTbpackag([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/tbpackag");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("tbpackag.mid")}  field = "mid" sortable key =  {10005}  ></Column>,
                            <Column header ={t("tbpackag.project")}  field = "project" sortable key =  {10006}  ></Column>,
                            <Column header ={t("tbpackag.product")}  field = "product" sortable key =  {10007}  ></Column>,
                            <Column header ={t("tbpackag.productCode")}  field = "productCode" sortable key =  {10008}  ></Column>,
                            <Column header ={t("tbpackag.code")}  field = "code" sortable key =  {10009}  ></Column>,
                            <Column header ={t("tbpackag.deliveryQuantity")} field = "deliveryQuantity" sortable key =  {100010}  ></Column>,
                            <Column header ={t("tbpackag.deliveryDate")} field = "deliveryDate" sortable key =  {100011}  ></Column>,
                            <Column header ={t("tbpackag.factoryOrderNumber")}  field = "factoryOrderNumber" sortable key =  {100012}  ></Column>,
                            <Column header ={t("tbpackag.trackingNumber")}  field = "trackingNumber" sortable key =  {100013}  ></Column>,
                            <Column header ={t("tbpackag.expressCompany")}  field = "expressCompany" sortable key =  {100014}  ></Column>,
                            <Column header ={t("tbpackag.recipient")}  field = "recipient" sortable key =  {100015}  ></Column>,
                            <Column header ={t("tbpackag.shipper")}  field = "shipper" sortable key =  {100016}  ></Column>,
                            <Column header ={t("tbpackag.photo")}  field = "photo" sortable key =  {100017}  ></Column>,
                          ...
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/tbpackag/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/tbpackag/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlTbpackag.removeTbpackag([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
