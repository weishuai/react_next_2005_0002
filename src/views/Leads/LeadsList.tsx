...
export const LeadsList = () => {
    ...
    let count = 0; 
    const fn=async ()=>{
        await ctlLeads.getLeadsAll(fhevent).then(data =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/leads/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    // ctlLeads.removeLeads([item.id]).then(()=>{});
                }
                   ...
                   fhHeight={'800px'}                    
                    FHColumns={
                        [
                            ...
                            <Column header={t('leads.name')} field="name" sortable  key={2}></Column>,
                            <Column header ={t("leads.company")}  field = "company" sortable key =  {10006}  ></Column>,
                            <Column header ={t("leads.industry")}  field = "industry" sortable key =  {10007}  ></Column>,
                            <Column header ={t("leads.email")}  field = "email" sortable key =  {10009}  ></Column>,
                            <Column header={t('leads.isactived')}  field="isactived"  body={(rowData) => {
                                let fhtxt="";
                            ...
                            <Column header={t('leads.updatedAt')} field="updatedAt" body={(rowData) => {
                                return(
                                    <React.Fragment>
                            ...
                            <Column  header={t('dataTable.Info')} field="id"   alignHeader="center" headerStyle={{minWidth: '14rem'}} style={{ width: '14rem'}}   body={(rowData) => {
                                return(
                                    <div style={{textAlign:'center',flex:1}}>
                                      <Button label={t('dataTable.HeadInfo')} style={{height:25,width:51}} className="p-button-sm p-button-info"  onClick={()=>{replace("/leads/"+rowData.id+"/2");}}></Button>
                                                                         
                                      <Button label={t('dataTable.Edit')}  style={{height:25,width:51}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/leads/"+rowData.id+"/1");}} />

                                      <Button label={t('dataTable.Delete')}  style={{height:25,width:51}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlLeads.removeLeads([rowData.id]).then(()=>{
                                             //replace("/leads");
                                            });
                                            const mynanoid: string = nanoid();
                                            //props.FHonChange(mynanoid);
