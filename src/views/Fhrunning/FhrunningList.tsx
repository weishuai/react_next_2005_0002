...
export const FhrunningList = () => {
    ...
    const ctlFhrunning = new FhrunningController();
    const ctlTiems=new TimesController();
    const { pathname, push,replace } = useRouter();
    const [val, setVal] = useState("");
    var fhcount=0;
    const fn=async (fhevent)=>{
        await   ctlFhrunning.getFhrunningAll(fhevent).then(data =>{

             ctlFhrunning.getFhrunningAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/fhrunning/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlFhrunning.removeFhrunning([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/fhrunning");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("fhrunning.fhname")}  field = "fhname" sortable key =  {10005}  ></Column>,
                            <Column header ={t("fhrunning.fhF1")}  field = "fhF1" sortable key =  {10006}  ></Column>,
                            <Column header ={t("fhrunning.fhF2")}  field = "fhF2" sortable key =  {10007}  ></Column>,
                            <Column header ={t("fhrunning.fhF3")}  field = "fhF3" sortable key =  {10008}  ></Column>,
                          
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/fhrunning/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/fhrunning/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlFhrunning.removeFhrunning([rowData.id]).then(()=>{
                                             // fn(fhevent);
                                            });
