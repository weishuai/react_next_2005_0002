...
export const QualityInspectionList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlQualityInspection.getQualityInspectionAll(fhevent).then(data =>{

             ctlQualityInspection.getQualityInspectionAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/qualityinspection/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlQualityInspection.removeQualityInspection([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/qualityInspection");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("qualityInspection.fhname")}  field = "fhname" sortable key =  {10005}  ></Column>,
                            <Column header ={t("qualityInspection.title")}  field = "title" sortable key =  {10006}  ></Column>,
                            <Column header ={t("qualityInspection.team")}  field = "team" sortable key =  {10007}  ></Column>,
                            <Column header ={t("qualityInspection.productTmlp")}  field = "productTmlp" sortable key =  {10008}  ></Column>,
                            <Column header ={t("qualityInspection.head")}  field = "head" sortable key =  {10009}  ></Column>,
                            <Column header ={t("qualityInspection.product")}  field = "product" sortable key =  {100010}  ></Column>,
                            <Column header ={t("qualityInspection.batch")}  field = "batch" sortable key =  {100012}  ></Column>,

                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/qualityInspection/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/qualityInspection/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlQualityInspection.removeQualityInspection([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
