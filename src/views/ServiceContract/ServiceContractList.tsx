...
export const ServiceContractList = () => {
    ...
    let count = 0;
    const fn=async ()=>{
        await   ctlServiceContract.getServiceContractAll(fhevent).then(data =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/servicecontract/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    // ctlServiceContract.removeServiceContract([item.id]).then(()=>{});
                }
           ...
           fhHeight={'800px'}                   
                    FHColumns={
                        [
                            ...
                            <Column header ={t("serviceContract.contractRef")}  field = "contractRef" sortable key =  {10005}  ></Column>,
                            <Column header ={t("serviceContract.contractName")}  field = "contractName" sortable key =  {10006}  ></Column>,
                            <Column header ={t("serviceContract.status")}  field = "status" 
                            ...
                            <Column header ={t("serviceContract.account")}  field = "account" sortable key =  {10009}  ></Column>,
                            ...
                            <Column  header={t('dataTable.Info')} field="id"   alignHeader="center" headerStyle={{minWidth: '14rem'}} style={{ width: '14rem'}}   body={(rowData) => {
                                return(
                                    <div style={{textAlign:'center',flex:1}}>
                                      <Button label={t('dataTable.HeadInfo')} style={{height:25,width:51}} className="p-button-sm p-button-info"  onClick={()=>{replace("/servicecontract/"+rowData.id+"/2");}}></Button>
                                                                         
                                     <Button label={t('dataTable.Edit')}  style={{height:25,width:51}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/servicecontract/"+rowData.id+"/1");}} />

                                     <Button label={t('dataTable.Delete')}  style={{height:25,width:51}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlServiceContract.removeServiceContract([rowData.id]).then(()=>{
                                            // replace("/servicecontract");
                                            });
