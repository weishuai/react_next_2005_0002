...
export const MyTbmatchingList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlMyTbmatching.getMyTbmatchingAll(fhevent).then(data =>{

             ctlMyTbmatching.getMyTbmatchingAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/mytbmatching/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlMyTbmatching.removeMyTbmatching([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/mytbmatching");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("mytbmatching.fhname")}  field = "fhname" sortable key =  {10005}  ></Column>,
                            <Column header ={t("mytbmatching.fhdate")} field = "fhdate" sortable key =  {10006}  ></Column>,
                            <Column header ={t("mytbmatching.sampleDate")} field = "sampleDate" sortable key =  {10007}  ></Column>,
                            // <Column header ={t("mytbmatching.documentNumber")}  field = "documentNumber" sortable key =  {10008}  ></Column>,
                            <Column header ={t("mytbmatching.customerName")} field = "customerName" sortable key =  {100010}  ></Column>,
                            <Column header ={t("mytbmatching.sampleName")}  field = "sampleName" sortable key =  {100011}  ></Column>,
                            <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/mytbmatching/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/mytbmatching/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlMyTbmatching.removeMyTbmatching([rowData.id]).then(()=>{
                                              fn(fhevent);
                                            });
                                         }}  />
                                    </React.Fragment>
