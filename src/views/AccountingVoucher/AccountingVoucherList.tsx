...
import { AccountingVoucherController } from '../../controllers/AccountingVoucherController';
...
export const AccountingVoucherList = () => {
    ...
    const ctlAccountingVoucher = new AccountingVoucherController();
    const ctlTiems=new TimesController();
    const { pathname, push,replace } = useRouter();
    const [val, setVal] = useState("");
    var fhcount=0;
    const fn=async (fhevent)=>{
        await   ctlAccountingVoucher.getAccountingVoucherAll(fhevent).then(data =>{
             ctlAccountingVoucher.getAccountingVoucherAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/accountingVoucher/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlAccountingVoucher.removeAccountingVoucher([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/accountingVoucher");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("accountingVoucher.title")}  field = "title" sortable key =  {10005}  ></Column>,
                            <Column header ={t("accountingVoucher.accountNumber")}  field = "accountNumber" sortable key =  {10006}  ></Column>,
                            <Column header ={t("accountingVoucher.type")}  field = "type" sortable key =  {10007}  ></Column>,
                            <Column header ={t("accountingVoucher.billReference")}  field = "billReference" sortable key =  {10008}  ></Column>,
                            <Column header ={t("accountingVoucher.statementDate")} field = "statementDate" sortable key =  {10009}  ></Column>,
                            <Column header ={t("accountingVoucher.accountingDate")} field = "accountingDate" sortable key =  {100010}  ></Column>,
  
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/accountingVoucher/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/accountingVoucher/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlAccountingVoucher.removeAccountingVoucher([rowData.id]).then(()=>{
                                            //   fn(fhevent);
                                            });
