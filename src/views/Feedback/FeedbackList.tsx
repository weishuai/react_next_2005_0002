...
export const FeedbackList = () => {
    ...
    let count = 0; 
    const fn=async ()=>{
        await   ctlFeedback.getFeedbackAll(fhevent).then(data =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/feedback/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    // ctlFeedback.removeFeedback([item.id]).then(()=>{});
                }
                replace("/feedback");
                }}   />
            <span style={{width:100}}></span>
                  ...
                  fhHeight={'800px'}                   
                    FHColumns={
                        [
                            ...
                            <Column header={t('feedback.name')} field="name" sortable  key={2}></Column>,

                            <Column header={t('feedback.description')}   field="description"  body={(rowData) => {
                                return(
                                    <React.Fragment>
                            ...
                            <Column header={t('feedback.isactived')}  field="isactived"  body={(rowData) => {
                                let fhtxt="";
                            ...
                            <Column header={t('feedback.createdAt')} field="createdAt" body={(rowData) => {
                                return(
                                    <React.Fragment>
                            ...
                            <Column header={t('feedback.updatedAt')} field="updatedAt" body={(rowData) => {
                                return(
                                    <React.Fragment>
                            ...
                            <Column  header={t('dataTable.Info')} field="id"   alignHeader="center" headerStyle={{minWidth: '14rem'}} style={{ width: '14rem'}}   body={(rowData) => {
                                return(
                                    <div style={{textAlign:'center',flex:1}}>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25,width:51}} className="p-button-sm p-button-info"  onClick={()=>{replace("/feedback/"+rowData.id+"/2");}}></Button>
                                  
                                    <Button label={t('dataTable.Edit')}  style={{height:25,width:51}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/feedback/"+rowData.id+"/1");}} />

                                    <Button label={t('dataTable.Delete')}  style={{height:25,width:51}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlFeedback.removeFeedback([rowData.id]).then(()=>{
                                             //replace("/feedback");
                                            });
