...
export const MyWorkList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlMyWork.getMyWorkAll(fhevent).then(data =>{

             ctlMyWork.getMyWorkAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/mywork/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlMyWork.removeMyWork([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/mywork");
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("mywork.operationName")}  field = "operationName" sortable key =  {10005}  ></Column>,
                            <Column header ={t("mywork.duration")}  field = "duration" sortable key =  {10006}  ></Column>,
                            <Column header ={t("mywork.workCenter")}  field = "workCenter" sortable key =  {10007}  ></Column>,
                            <Column header ={t("mywork.defaultDuration")} field = "defaultDuration" sortable key =  {10008}  ></Column>,
     
                           <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/mywork/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/mywork/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlMyWork.removeMyWork([rowData.id]).then(()=>{
                                              fn(fhevent);
                                            });
                                         }}  />
                                    </React.Fragment>
