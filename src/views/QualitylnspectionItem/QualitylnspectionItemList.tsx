...
export const QualitylnspectionItemList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlQualitylnspectionItem.getQualitylnspectionItemAll(fhevent).then(data =>{

             ctlQualitylnspectionItem.getQualitylnspectionItemAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/qualitylnspectionitem/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlQualitylnspectionItem.removeQualitylnspectionItem([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/qualitylnspectionItem");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("qualitylnspectionItem.fhitem")}  field = "fhitem" sortable key =  {10006}  ></Column>,
                            <Column header ={t("qualitylnspectionItem.fhtxt")}  field = "fhtxt" sortable key =  {10007}  ></Column>,
                            <Column header ={t("qualitylnspectionItem.fhnotes")}  field = "fhnotes" sortable key =  {10008}  ></Column>,


                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/qualitylnspectionItem/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/qualitylnspectionItem/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlQualitylnspectionItem.removeQualitylnspectionItem([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
