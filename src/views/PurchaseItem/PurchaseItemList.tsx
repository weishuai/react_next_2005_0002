...
export const PurchaseItemList = (props: any) => {
    ...
    const ctlPurchaseItem = new PurchaseItemController();
    ...
    const fn=async ()=>{
        await   ctlPurchaseItem.getPurchaseItemAll(fhevent).then(data =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/purchaseitem/0/0/"+Mainid);}} style={{backgroundColor:'#4682B4'}}  />
         
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlPurchaseItem.removePurchaseItem([item.id]).then(()=>{});
                }
              ...
              fhHeight={'800px'}                   
                    FHColumns={
                        [
                            ...
                            <Column header={t('purchaseItem.name')} field="name" sortable  key={2}></Column>,
                            <Column header ={t("purchaseItem.qty")} field = "qty" sortable key =  {10005}  ></Column>,
                            <Column header ={t("purchaseItem.balanceQty")} field = "balanceQty" sortable key =  {10006}  ></Column>,
                            <Column header ={t("purchaseItem.unit")} field = "unit" sortable key =  {10007}  ></Column>,
                            <Column header={t('purchaseItem.updatedAt')} field="updatedAt" body={(rowData) => {
                                return(
                                    <React.Fragment>
                            ...
                            <Column  header={t('dataTable.Info')} field="id"   alignHeader="center" headerStyle={{minWidth: '14rem'}} style={{ width: '14rem'}}   body={(rowData) => {
                                return(
                                    ...
                                    <Button label={t('dataTable.HeadInfo')} style={{height:25,width:51}} className="p-button-sm p-button-info"  onClick={()=>{replace("/purchaseitem/"+rowData.id+"/2/"+props.mid);}}></Button>
                                                                           
                                     <Button label={t('dataTable.Edit')}  style={{height:25,width:51}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/purchaseitem/"+rowData.id+"/1/"+props.mid);}} />

                                     <Button label={t('dataTable.Delete')}  style={{height:25,width:51}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlPurchaseItem.removePurchaseItem([rowData.id]).then(()=>{
