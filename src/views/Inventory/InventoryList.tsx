...
export const InventoryList = () => {
    ...
    const ctlInventory = new InventoryController();
    const ctlTiems=new TimesController();
    const { pathname, push,replace } = useRouter();
    const [val, setVal] = useState("");
    var fhcount=0;
    const fn=async (fhevent)=>{
        await   ctlInventory.getInventoryAll(fhevent).then(data =>{

             ctlInventory.getInventoryAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/inventory/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlInventory.removeInventory([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/inventory");
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("inventory.title")}  field = "title" sortable key =  {10005}  ></Column>,
                            <Column header ={t("inventory.position")}  field = "position" sortable key =  {10006}  ></Column>,
                            <Column header ={t("inventory.jobDate")} field = "jobDate" sortable key =  {10007}  ></Column>,
                            <Column header ={t("inventory.warehouse")}  field = "warehouse" sortable key =  {10008}  ></Column>,
                          ...
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/inventory/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/inventory/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlInventory.removeInventory([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
