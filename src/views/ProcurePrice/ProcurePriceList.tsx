...
export const ProcurePriceList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlProcurePrice.getProcurePriceAll(fhevent).then(data =>{

             ctlProcurePrice.getProcurePriceAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/procureprice/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlProcurePrice.removeProcurePrice([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/pages/procurePrice");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("procurePrice.supplier")}  field = "supplier" sortable key =  {10005}  ></Column>,
                            <Column header ={t("procurePrice.product")}  field = "product" sortable key =  {10006}  ></Column>,
                            <Column header ={t("procurePrice.productCode")}  field = "productCode" sortable key =  {10007}  ></Column>,
                            <Column header ={t("procurePrice.deliveryLeadTime")} field = "deliveryLeadTime" sortable key =  {10008}  ></Column>,
                            <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/procurePrice/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/procurePrice/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlProcurePrice.removeProcurePrice([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
