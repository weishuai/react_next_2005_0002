...
export const BankCashList = () => {
    ...
    const fn=async (fhevent)=>{
        await   ctlBankCash.getBankCashAll(fhevent).then(data =>{

             ctlBankCash.getBankCashAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/bankCash/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlBankCash.removeBankCash([item.id]).then(()=>{
                     fn(fhevent);
                    });
                }
                //replace("/bankCash");
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("bankCash.report")}  field = "report" sortable key =  {10005}  ></Column>,
                           <Column header ={t("bankCash.refer")}  field = "refer" sortable key =  {10008}  ></Column>,
                            <Column header ={t("bankCash.partners")}  field = "partners" sortable key =  {10009}  ></Column>,
                            <Column header ={t("bankCash.amountMoney")} field = "amountMoney" sortable key =  {100010}  ></Column>,
                            <Column header ={t("bankCash.type")}  field = "type" sortable key =  {100013}  ></Column>,
                            <Column header ={t("bankCash.accountingVoucher")}  field = "accountingVoucher" sortable key =  {100014}  ></Column>,


                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/bankCash/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/bankCash/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlBankCash.removeBankCash([rowData.id]).then(()=>{
                                             // fn(fhevent);
                                            });
