...
export const TbmatchinglineList = () => {
    ...
    const ctlTbmatchingline = new TbmatchinglineController();
    const ctlTiems=new TimesController();
    const { pathname, push,replace } = useRouter();
    const [val, setVal] = useState("");
    var fhcount=0;
    const fn=async (fhevent)=>{
        await   ctlTbmatchingline.getTbmatchinglineAll(fhevent).then(data =>{

             ctlTbmatchingline.getTbmatchinglineAll_count(fhevent).then(dataok =>{
    ...
    return (
       ...
       <div >
         ...
         <span className="p-buttonset">
            ...
            <Button label={t('dataTable.Save')} icon="pi pi-check"  onClick={()=>{replace("/pages/tbmatchingline/0/0");}} style={{backgroundColor:'#4682B4'}}  />
            <span style={{width:100}}></span>
            <Button label={t('dataTable.Delete')} icon="pi pi-trash" style={{backgroundColor:'#4682B4'}} onClick={()=>{
                for(let item of FHids)
                {
                    ctlTbmatchingline.removeTbmatchingline([item.id]).then(()=>{
                    //  fn(fhevent);
                    });
                }
                //replace("/pages/tbmatchingline");
                }}   />
            <span style={{width:100}}></span>
            ...
            <MyDataTable
                    ...
                    FHColumns={
                        [
                            ...
                            <Column header ={t("tbmatchingline.mid")}  field = "mid" sortable key =  {10005}  ></Column>,
                            <Column header ={t("tbmatchingline.rawMaterial")}  field = "rawMaterial" sortable key =  {10006}  ></Column>,
                            <Column header ={t("tbmatchingline.batchNumber")}  field = "batchNumber" sortable key =  {10007}  ></Column>,
                            <Column header ={t("tbmatchingline.number")} field = "number" sortable key =  {10008}  ></Column>,
                            <Column header ={t("tbmatchingline.notes")}  field = "notes" sortable key =  {10009}   ></Column>,
                          ...
                          <Column  header={t('dataTable.Info')} field="id"   body={(rowData) => {
                                return(
                                    <React.Fragment>
                                     <Button label={t('dataTable.HeadInfo')} style={{height:25}} className="p-button-sm p-button-info"  onClick={()=>{replace("/pages/tbmatchingline/"+rowData.id+"/2");}}></Button>
                                     <Button label={t('dataTable.Edit')}  style={{height:25}}  className="p-button-sm p-button-secondary"  onClick={()=>{replace("/pages/tbmatchingline/"+rowData.id+"/1");}} />
                                     <Button label={t('dataTable.Delete')}  style={{height:25}}  className="p-button-sm p-button-danger" onClick={() => {
                                         ctlTbmatchingline.removeTbmatchingline([rowData.id]).then(()=>{
                                              //fn(fhevent);
                                            });
